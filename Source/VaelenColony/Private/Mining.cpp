...
namespace Vaelen::Colony
{
	...
	bool FoundColony(World& W, const History::PreHistoryTypes& Types, const ColonyTypes& Colony, uint32 Region)
	{
		...
		W.Events().Publish(W.Now(), ColonyFoundedEvent, Economy::StockPayload{Region, 0u, G_ORE, 0u},
						   W.Entities().GetId(RH));
		return true;
	}
	...
	{
		if (Context.Events == nullptr || Rules.DaysPerYear == 0)
		{
			return;
		}
		World& W = *Owner;
		// Every colony the world holds, and not one named by a rule: a colony is
		// founded at a tick and a rule is fixed when the system is built, so a
		// rule cannot name one (ADR-0090). Walking the colony pool also walks
		// exactly as many entities as there are colonies, which is why this can
		// run every day at all.
		std::vector<std::pair<EntityHandle, uint32>> Colonies;
		W.Components()
		...
		// a year of days lifts what a year should, and a colony cannot drift from
		// the world around it by rounding.
		const uint64 Day = Context.Tick / 24u;
		for (const auto& [RH, Region] : Colonies)
		{
			// The hands, and what a year of them would lift. Mining has no skill
			// of its own: Skill::Count sizes PersonTraits, so a sixth skill would
			// move the component digest of every world ever made. The colony's
			// hands use the craft 06.02 already turns ore into tools with.
			uint32 Hands = 0;
			uint64 YearTotal = 0;
			W.Components()
				...
				.ForEach(
					[&](EntityHandle H, const Population::PersonInfo& P)
					{
						...
						const uint64 Age =
							Context.Tick > P.Born ? (Context.Tick - P.Born) / History::TicksPerYear : 0ull;
			...
			if (YearTotal == 0)
			{
				continue; // nobody old enough, or a rule that lifts nothing
			}
			uint32 Take =
				Population::ShareOfDay(static_cast<uint32>(Day % Rules.DaysPerYear), Rules.DaysPerYear,
			...
			const std::vector<Seam> Seams = SeamsOf(W, Types, Region);
			uint32 Lifted = 0;
			...
			for (const Seam& S : Seams)
			{
				...
				if (T->Taken >= S.Richness)
				{
					Context.Events->Publish(Context.Tick, SeamWorkedOutEvent,
											Economy::StockPayload{Region, S.Index, G_ORE, 0u}, Subject);
				}
			}
			if (Lifted == 0)
			{
