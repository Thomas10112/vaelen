...
Source/
  ...
  VaelenMilitary/                KERNEL (Phase 08): levies, armies, marching, battle, siege, war, the toll; Public/Vaelen/Military/*.h
  VaelenInfrastructure/          KERNEL (Phase 09): buildings raised out of a region's common stock; Public/Vaelen/Infrastructure/*.h
  VaelenPlayer/                  KERNEL (Phase 10): the player as a mark on one person the world already had; Public/Vaelen/Player/*.h
...
| Military module (08.01 levies and armies: people taken out of the regions a polity holds, fed out of its treasury, melting away when they are not; 08.02 marching: a host walks the region graph towards the nearest enemy host, one hop a season, eats off what it stands on and loosens an enemy ruler's grip; 08.03 battle: two hosts on one region settled in a year by strength, whose ground it is and a stream fixed by the world seed, the loser falling back or gone; 08.04 siege: a host sitting before an enemy seat brings its wall down at the rate of the men before it, and a polity that loses its seat ends; 08.05 war as a thing with a beginning and an end: opened by a relation turning, held open while it runs, closed by exhaustion, terms being whatever has already changed hands; 08.06 what war costs the living: the fallen struck out where they came from, the standing of those who came back, and the people who would not live under a host; 08.07 war in the chronicle: a sentence for every military event and the why of a lost province walked back to the battle that caused it; 08.08 the phase gate) | `Source/VaelenMilitary` | VALIDATED (08.01-08.08, Phase 08 closed) headless; UNVERIFIED under UBT |
| Infrastructure module (09.01 buildings: things standing in a region, raised out of what it holds in common and the hands its fields can spare, one work of each kind made bigger as the region grows, every unit of timber, tools and grain taken with the raising as its cause; 09.02 what a building does: a granary softening the drought, a mill and a smithy lifting the harvest and the craft, a wall slowing what a siege brings down - each of them the one number an earlier phase already reads; 09.03 settlements as places: the settlement of 06.04 given a body, one tile of its region chosen once and never moved, a size from the people around it and the goods through it, and the region's works standing in the town where there is one; 09.04 roads: a route of 06.04 cut out of the two ends together where enough already crosses, letting more of the trade that already wanted to happen get across, worn when nobody keeps it and fallen back to a track; 09.05 decay and ruins: every work wearing at the rate of what it is, faster where a flood or a host struck, mended out of the common stock or lost, and the ruin it leaves making the next one cheaper; 09.06 logistics: one number on the ground written from the best road touching it, read by the marching of 08.02 and the reach of 07.03; 09.07 infrastructure in the chronicle: a granary raised, a mill fallen in, a road cut, a town settled, the fall naming the flood that finished it, and the text falling through to every layer below; 09.08 the phase gate) | `Source/VaelenInfrastructure` | VALIDATED (09.01-09.08, Phase 09 closed) headless; UNVERIFIED under UBT |
| Player module (10.01 the player as a mark on an existing person of a detailed region: one to a world, carrying no state of its own, so that a world with a player in it runs exactly as a world without one until the player actually does something; 10.02 the enslaved start: a life found among the people 05.04 had already bound rather than written into the world, on ore ground where the world offers one there and honest about it when it does not; 10.03 the player's grain: the first system in nine phases to run at the day rather than the year, granting waking hours to one person and publishing nothing, so the world's history is the same whether somebody is living in it or not; 10.04 intent as commands: what arrives from outside the simulation is queued and changes nothing at all, and a system inside it is the only thing allowed to act on one, which is what makes a recorded stream of intents replay to the same life; 10.05 what the player can do: work, rest, eat, move, speak, give and take, each one a call into the module that already owns that change and never a field set from here, so that sixty days of a life replay to the same world with the economy and the people in it; 10.06 what the people around them make of them: opinions read out of the acts in the event log rather than out of a dialogue tree, weighed by the standing 05.02 gives whoever holds them, and forgotten again when nothing more happens; 10.07 the player in the chronicle: a life as records of what touched somebody else, every event with a sentence that falls through to the layer that owns it, and the why of the grain that moved walked back to the giving that moved it; 10.08 the gate: forty years at 256 played across three lives out of a recorded stream, replayed into a fresh world of the same seed to the same digest, the same log and the same life) | `Source/VaelenPlayer` | VALIDATED headless (Phase 10 closed, CI run 108: nine jobs green); UNVERIFIED under UBT |
