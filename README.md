...
| Player module (10.01 the player as a mark on an existing person of a detailed region: one to a world, carrying no state of its own, so that a world with a player in it runs exactly as a world without one until the player actually does something; 10.02 the enslaved start: a life found among the people 05.04 had already bound rather than written into the world, on ore ground where the world offers one there and honest about it when it does not) | `Source/VaelenPlayer` | PROTOTYPE (10.01-10.02) headless; UNVERIFIED under UBT |
