...
| Player module (10.01 the player as a mark on an existing person of a detailed region: one to a world, carrying no state of its own, so that a world with a player in it runs exactly as a world without one until the player actually does something) | `Source/VaelenPlayer` | PROTOTYPE (10.01) headless; UNVERIFIED under UBT |
