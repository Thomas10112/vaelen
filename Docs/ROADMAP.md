...
**NOT DONE: the stream checked into `Tests/Run/Streams/` with its README line.** `Atlas --walk` refuses to write a walk that misses the gate's clauses — correctly, and for the reason 14.10 records — so a death walk needs a write mode of its own, plus `--from-age`/`--to-age` on the tool, since `--savefuzz` today replays with the default 16–40 rules and would re-take a different person. That is a real piece of work and not a line of glue, so it is named here rather than half-built. `Run.SaveDeath` carries the clause's substance without it.
