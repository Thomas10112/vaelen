...
| 15.05 | The vanishing believer of finding 4, and any sibling the audit turns up. | The audit's faith gap is 0 over a 120-year world that crosses into full neighbours; the mutation that removes the fix makes it non-zero. | **INCOMPLETE** — the fix is in and the precondition and invariant are pinned by CTest `Lod.AFifthFaithIsRefusedAndTheCrossingNowReadsTheAnswer`, but the crossing path itself is NOT covered: three ways of staging it were tried and each was defeated by the pre-history's religion systems rewriting faith tables before the crossing runs. The test says so in full. What it wants is a harness that runs the bridge without them, and that is the rest of this task |
