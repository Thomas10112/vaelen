...
**AND THE TASK FOUND THAT NOTHING HAD EVER BEEN PLAYED IN THESE TESTS.** `TakeUp` returned 0 and the calls did not check it. Measured across sizes 32 and 64 and histories of 10, 30 and 60 years: **`Options::Lively` is what makes anybody playable** — without it the world offers nobody, every time; at 6+6 years it offers nobody even with it. Two Phase 16 tests were setting up a "played" world and running on an unplayed one, and the 6+6 control had two `TakeUp` calls that did nothing at all. Both are fixed and the takings are checked now.
