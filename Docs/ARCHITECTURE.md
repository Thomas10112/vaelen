...
| `VAELENCORE_API` | Generated by UBT (`<MODULE>_API` export/import macro) | Defined empty: `VAELENCORE_API=` on `vaelen_build_flags`; `CoreTypes.h` also has an `#ifndef VAELENCORE_API` fallback to empty |
| Build define | `VAELEN_UNREAL_BUILD=1` (`VaelenCore.Build.cs`, `PublicDefinitions`) | `VAELEN_HEADLESS_BUILD=1` |
| Assertions | `VAELEN_ASSERTS_ENABLED` derived in `Assert.h`: 0 in `UE_BUILD_SHIPPING`, else 0 if `NDEBUG`, else 1 | `VAELEN_ASSERTS_ENABLED=1` or `=0` from option `VAELEN_ENABLE_ASSERTS` (default ON) |
| Log compile floor | `VAELEN_LOG_COMPILED_MIN_LEVEL` = Info in `UE_BUILD_SHIPPING`, else Trace (`Log.h`) | Trace (no override) |
| Exceptions / RTTI | `bEnableExceptions = false`, `bUseRTTI = false` | `-fno-exceptions -fno-rtti`; MSVC: `/GR-`, `_HAS_EXCEPTIONS=0`, `/EHsc` stripped |
| PCH / unity | `PCHUsage = NoPCHs`, `bUseUnity = false` | n/a |
| Language | `CppStandardVersion.Cpp20` | `CMAKE_CXX_STANDARD 20`, extensions OFF |
| Warnings | UBT defaults (not verified: no engine build has been run) | `-Wall -Wextra -Wpedantic -Wshadow -Wconversion -Wsign-conversion -Wold-style-cast -Wcast-align -Wnon-virtual-dtor -Woverloaded-virtual -Wnull-dereference -Wdouble-promotion -Wformat=2`, `-Werror` when `VAELEN_WARNINGS_AS_ERRORS` (default ON); MSVC `/W4 /permissive- /utf-8 /WX` |
...
| Status | UNVERIFIED (never compiled by UBT in this repository's history) | VALIDATED: clang 18 and gcc 13 (Linux, Debug), 112 tests, 0 warnings |

Where the defines are consumed today: `VAELEN_UNREAL_BUILD` in `Assert.h` (line 23) and
`Log.h` (line 106); `VAELENCORE_API` on every exported symbol; `VAELEN_ASSERTS_ENABLED`
in `Assert.h` and `Tests/Core/Test_Assert.cpp`. `VAELEN_HEADLESS_BUILD` is defined by
CMake but not read by any source yet.
...
flowchart BT
    subgraph Kernel["Kernel modules - engine-agnostic, compiled by UBT and CMake"]
        Core["VaelenCore"]
        Sim["VaelenSim (PLANNED)"]
        World["VaelenWorld (PLANNED)"]
        History["VaelenHistory (PLANNED)"]
        Population["VaelenPopulation (PLANNED)"]
    ...
    class Core,Bridge existing;
    class Sim,World,History,Population,Society,Economy,Politics,Military,Infrastructure,Persistence,DevTools,Modding,Game,Presentation,UI planned;
```
...
All files below carry `// STATUS: VALIDATED (Phase 00)` in their header. Test counts are
from `VaelenCoreTests --list` (112 tests, 21 785 checks; all pass with clang 18 and gcc 13
in Debug with assertions enabled).

### CoreTypes (`CoreTypes.h`) - VALIDATED

Fixed-width aliases `int8..int64`, `uint8..uint64`, `usize` in namespace `Vaelen`;
static asserts on 64-bit integers and IEEE-754 float sizes. `NonCopyable` base,
`ArrayCount`, `ToUnderlying`. Provides the empty `VAELENCORE_API` fallback. No `.cpp`.
Tested indirectly by every suite.
...
- Purity check: `Tools/check_kernel_purity.py --root <repo>` scans every module in
  ...
  runs 32 checks on synthetic modules. Current result: 12 files, 0 violations.
...
(ON). Requires CMake >= 3.24 and a C++20 compiler; Python 3 for `Kernel.Purity`.
...
|---|---|---|---|---|
| Linux | `ubuntu-24.04` (installs ninja, clang, g++, cmake, python3) | `linux-clang-debug`, `linux-clang-release`, `linux-gcc-debug`, `linux-gcc-release` (matrix, `fail-fast: false`) | Ninja | Debug / RelWithDebInfo |
| Windows | `windows-2022` | configure `windows-msvc`, build and test `windows-msvc-debug` | Visual Studio 17 2022, x64 | Debug |
| macOS | `macos-14` (installs ninja) | `macos-debug` | Ninja | Debug |

Every configure preset carries a host-system condition (Linux, Darwin or Windows), so
it is ignored on other hosts. CTest entries: `Kernel.Purity`, `Core.Assert`,
`Core.Harness`, `Core.Hash`, `Core.Ids`, `Core.Log`, `Core.Random`, `Core.Version`
(8 in total).

Verified in this repository: the Linux clang and gcc Debug configurations, invoked with
the plain `cmake -S/-B` commands above (same generator, build type and default options
as the `linux-clang-debug` / `linux-gcc-debug` presets): 8/8 CTest entries pass,
112/112 tests. The Windows, macOS and Release legs are defined in the workflow but
their results have not been observed from inside this repository.
...
(requires UE5).` in its header. Their content was read but not compiled, linked or
executed.

| File | Claimed role |
|---|---|
| `Source/VaelenCore/Private/VaelenCoreModule.cpp` | `FVaelenCoreModule : IModuleInterface`, `IMPLEMENT_MODULE(FVaelenCoreModule, VaelenCore)`, `IsGameModule() = false` |
| `Source/VaelenCore/VaelenCore.Build.cs` | Module rules described in section 2 |
| `Source/Vaelen/Vaelen.Build.cs` | Primary game module rules |
| `Source/Vaelen/Public/Vaelen.h` | `FVaelenModule`, `LogVaelen` category (`VAELEN_API`) |
| `Source/Vaelen/Private/Vaelen.cpp` | Installs `FVaelenLogSink` and the Unreal assertion handler, logs `VAELEN <version> - kernel save format v<n> - module started`, `IMPLEMENT_PRIMARY_GAME_MODULE` |
| `Source/Vaelen/Private/VaelenLogSink.h/.cpp` | Maps `LogLevel` Trace/Debug/Info/Warning/Error/Fatal to `VeryVerbose/Verbose/Log/Warning/Error/Error` |
| `Source/Vaelen.Target.cs`, `Source/VaelenEditor.Target.cs` | Game and Editor targets, `BuildSettingsVersion.V5`, include order `Unreal5_6`, C++20 |
| `Vaelen.uproject` | Module list and load order, plugin `ModelingToolsEditorMode` (Editor only) |
| `Config/Default*.ini` | Engine, game (`ProjectVersion=0.0.1`), editor and input defaults |

Also unverified: that UBT accepts the kernel sources with its own warning settings, and
that `VAELENCORE_API` as generated by UBT is compatible with every use in the headers
(for instance on the constexpr `GetSaveFormatVersion()`). Verification path: install
UE 5.6, generate project files, build `VaelenEditor` (Development), open the project and
confirm the module-started log line.
...
- Style: `.clang-format` (LLVM base, Allman braces, tabs, width 4, column limit 120,
  pointer alignment left) and `.editorconfig` (tabs for `.h/.cpp/.inl/.cs`, 2 spaces
  for docs, YAML, JSON, CMake, Python). Naming is PascalCase for types, functions and
  members; macros are `VAELEN_*`.
...
Found while cross-checking code, comments and this document; reported, not fixed here.

1. `.github/workflows/kernel-ci.yml` (line 2-3) says the Unreal project "is built on an
   engine-backed runner"; no such runner or workflow exists (section 7.2).
2. `Config/DefaultEditor.ini` refers to a README; there is no `README.md` in the
   repository.
3. `Random.h` documents `RangeInclusive` as "Unbiased (Lemire)"; `Random.cpp::Below`
   explicitly uses bitmask-with-rejection and states that Lemire's method was not
   chosen. The result is unbiased; the algorithm name in the header is wrong.
4. `Log.h` says "The headless build installs a stdout sink"; `TestMain.cpp` installs
   `StdioLogSink` only when `--verbose` is given (`--quiet-log` is the default).
5. `VAELEN_HEADLESS_BUILD=1` is defined by CMake but not consumed by any source.
6. The purity checker accepts a fourth status value, UNVERIFIED, in addition to the three
   labels defined by the project specification; the Unreal-facing files use it.
7. `Tests/Harness/VaelenTest.h` says `ScopedAssertCapture` "restores the previous handler
   on destruction"; its destructor calls `SetAssertHandler(nullptr)`, which installs the
   default handler, not the previously installed one.

## 12. Sources checked

`/CMakeLists.txt`, `/CMakePresets.json`, `/Vaelen.uproject`, `/.github/workflows/kernel-ci.yml`,
`/.clang-format`, `/.editorconfig`, `/.gitignore`, `Config/Default{Engine,Game,Editor,Input}.ini`,
`Source/Vaelen.Target.cs`, `Source/VaelenEditor.Target.cs`, `Source/VaelenCore/{VaelenCore.Build.cs,CMakeLists.txt}`,
...
`Tests/Core/Test_*.cpp` (headers, includes and test names), `Tools/check_kernel_purity.py`,
`Tools/kernel_modules.txt`. Builds and test runs executed: clang++ 18.1.3 and g++ 13.3.0,
Linux, Debug, `cmake` 3.28.3, Ninja; `ctest` 8/8; `VaelenCoreTests` 112/112; purity checker
`12 files, 0 violations`, self-test `32 checks, 0 failed`.
