...
| `VaelenInfrastructure` | Kernel (UBT Runtime module, `PreDefault`; CMake static library), Phase 09 | SIMULATION (buildings, settlements, roads, decay, logistics) | UBT: `Core`, `VaelenCore`, `VaelenSim`, `VaelenPopulation`, `VaelenSociety`, `VaelenEconomy`, `VaelenPolitics`, `VaelenMilitary`. CMake: `Vaelen::Military` and below. | 09.01-09.07 PROTOTYPE headless; UNVERIFIED under UBT |
