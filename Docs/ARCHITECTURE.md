...
| 14 UI | `VaelenUI` | Unreal | UI, read-only view plus command submission through `VaelenGame`. Written by 14.09; since 14.07 its includes are fenced by `Tools/check_ui_fence.py` (CTest `Kernel.UiFence`) and parsed with a restricted include set, against `Tools/UiWitness` until the module exists. |
