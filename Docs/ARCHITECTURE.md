...
| 11 MINING COLONY | `VaelenColony` | Kernel | Exists: the module, and a region the world keeps detailed by policy rather than by crowding (11.01); a colony's people living at the day while the world around them keeps the year, by a schedule whose days sum to the year exactly (11.02); the work of a colony - hands on the ore seams of 02.07, the seam running out as it is taken, and the ore credited through Economy::AddStock and nothing else so that the chronicle can say where it came from, with 06.02 told to stop reaping ground that is worked for what is under it (11.03); and who holds it - a colony founded bound rather than grown into bondage, held by the colony itself because the region's elite could never carry that many, with the overseers of 05.01 as an institution that outlives the people in it (11.04). |
...
| 10 PLAYER, 11 MINING COLONY, 12 GAMEPLAY | `VaelenGame` | Unreal | Player, input, gameplay commands; the mining colony start is content in the domain modules plus `VaelenGame`, not a separate module. Written by 14.08 (`UVaelenWorldSubsystem`, four console commands, no `Tick`): its PUBLIC headers are fenced with the UI's since 14.07, its `.cpp` deliberately not - it is the module that holds the world. UNVERIFIED until UnrealBuildTool has built it. |
...
| 14 UI | `VaelenUI` | Unreal | UI, read-only view plus command submission through `VaelenGame`. Written by 14.09 (`AVaelenHUD` drawing `Lines()` of the page, `AVaelenPlayerController` binding the letters the page carries, `AVaelenGameMode`): its includes are fenced by `Tools/check_ui_fence.py` (CTest `Kernel.UiFence`) and it is parsed with a restricted include set that holds no kernel header but the view leaves. UNVERIFIED until UnrealBuildTool has built it. |
...
/
  ...
  Source/
    ...
    VaelenView/                   KERNEL MODULE (Phase 13): Public/Vaelen/View/*.h (nine leaves + Take.h, 14.02/14.04/14.05/14.06), Private/*.cpp, VaelenView.Build.cs, CMakeLists.txt
