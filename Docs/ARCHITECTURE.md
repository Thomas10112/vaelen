...
| `VaelenMilitary` | Kernel (UBT Runtime module, `PreDefault`; CMake static library), Phase 08 | SIMULATION (levies, armies, battle, siege, war) | UBT: `Core`, `VaelenCore`, `VaelenSim`, `VaelenPopulation`, `VaelenSociety`, `VaelenEconomy`, `VaelenPolitics`. CMake: `Vaelen::Politics` and below. | 08.01 VALIDATED headless; UNVERIFIED under UBT |
...
| 08 MILITARY | `VaelenMilitary` | Kernel | Exists: the module, levies raised from the regions a polity holds and fed out of its treasury, armies as entities that melt when they are not fed (08.01). Planned: marching, battle, siege, war as a thing with an end, what war costs the living, war in the chronicle, the gate (08.02-08.08). |
