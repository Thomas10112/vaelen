...
**Status:** APPLIED 2026-09-25 by tasks 18.01, 18.02 and 18.10 (proposed 2026-09-24 by the Phase 18 panel, `Docs/ROADMAP.md` section 25). The prediction table and the census read against it are below.
...
### The prediction table, written 2026-09-25 BEFORE the flip was built or run (task 18.10)

The flip changes one default (`Options::Climate` and the Atlas's `--climate`
become on) and adds the warmth and the winter to the two engine actors. Only a
world built THROUGH Run::Aelvor or Tools/Atlas with the default changes; every
suite that wires its own world (Population, Economy, Society, Politics,
Military, Infrastructure, Player, Gameplay, Colony, Sim) declares nothing new
and runs nothing new. Predicted, by class and site:

| site | kind | prediction | why |
|---|---|---|---|
| Tests/CMakeLists.txt `Atlas.Frozen128` frame | WORLD/frame | MOVES | the default Atlas world now has a winter, a season and a warmth type |
| Tests/CMakeLists.txt ground `0x8f7f4948f49b6e86` (Frozen128, ClimateFrozen128) | WORLD/ground | UNMOVED | the map is generation's; nothing of Phase 18 writes a tile |
| Tests/CMakeLists.txt `Atlas.ClimateFrozen128` climate `0x59615fa1f5d24bd1` | WORLD/digest | UNMOVED | the leaf reads the map and the date only |
| Tests/CMakeLists.txt `Atlas.PanelFrozen` | WORLD/panel | MOVES | the page gains the weather row |
| Tests/Run/Test_Aelvor.cpp `VAELEN_RUN_FROZEN_FRAME` | WORLD/frame | MOVES | Options{} is a climate world |
| Tests/Run/Test_Aelvor.cpp `VAELEN_RUN_FROZEN_GROUND` | WORLD/ground | UNMOVED | as above |
| Tests/View (ViewGate, Panel, anything taken through Aelvor) | WORLD/frame, panel | MOVE where built through Aelvor | as above |
| Tests/Run/Containers corpus + Test_Containers | WORLD/section, trailer | MOVE | regenerated by its recorded command, which now runs a climate world; the HOST byte reads 1 |
| Tests/Run/Test_Climate.cpp `VAELEN_CLIMATE_LOG_128` | WORLD/log | UNMOVED | both of its worlds name the option explicitly |
| Replay.Played/Walked/Lived, Run.Gate(.Lived), Run.SaveFuzz* | WORLD (CMake) | UNMOVED | the recorded months replay with `--no-climate` (ERA in the four drivers) |
| Tests/Run/Golden/* and `Golden.*` | WORLD/RECORD | UNMOVED | `OptionsOf` sets `Climate = false`; the images are of the pre-winter era and are never regenerated |
| every GEN site (14), every HASH site | GEN, HASH | UNMOVED | generation and the event-type table are untouched |
| every suite that wires its own world | WORLD | UNMOVED | nothing is declared or run there |

The census diff over the flip commit is read against this table; any row it
prints that the table does not predict is a finding, written below after the
run.

### The census over the flip, read against the table (2026-09-25)

`python3 Tools/frozen_census.py --diff 3cc1831 <18.10>`: 24 removed, 33 added,
in 7 files - WORLD -14/+22 (frame -1/+2, panel/state -2/+3, section -6/+6,
trailer -5/+6, and the new pins), RECORD -10/+11 (the container README's
tables and one comment); **GEN, HASH, CONST, SEED and SYNTHETIC unmoved.**

Every removed site is in a row the table predicted: `Atlas.Frozen128`'s frame
(`abc5a5767c6cf9dd` -> `ec18241b89c3d246`, the same value in `Test_Aelvor`),
the two pages of `Test_Panel` and `Atlas.PanelFrozen` (EMPTY `54787451e65766c1`
-> `d7e7149e65ceb69a`, PLAYED `703c838ca533a095` -> `777351768a3a4fc6`), and
the corpus (three trailers, six section digests, in the test and the README).
Unmoved by name, as predicted: the ground `8f7f4948f49b6e86`; the climate leaf
`59615fa1f5d24bd1`; `VAELEN_CLIMATE_LOG_128` (and the empty play's log under
the flip IS that value, `c5acefba48cd0fb5` - the gate's climate world and the
default world are one world); every golden; every recorded month and walk
under `--no-climate`; every gate that wires its own world.

The additions the table did not list as MOVES are all new pins, none a moved
one: `Replay.Climate`'s four digests (the climate month, a new entry the row
asked for), and `Atlas.Frozen128Before` / `Atlas.PanelFrozenBefore`, which hold
the OLD frame and page under `--no-climate` - the world before, kept.
