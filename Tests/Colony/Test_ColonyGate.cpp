// VAELEN - Tests/Colony
// Phase 11.08: the Phase 11 gate.
//
// A century at 256 with the mining colony at full detail while the world around
// it keeps the year: every invariant of every phase from 04 to 11 checked each
// decade, the whole thing frozen, and a life played inside the colony replayed
// into a fresh world of the same seed to the same life.
//
// The harness is the Phase 10 gate's - the whole stack, which is what a gate
// needs - with the colony of Phase 11 founded inside it.
//
...
VAELEN_TEST(ColonyGate, ACenturyOfAColonyHoldsEveryInvariantAndReplays)
{
	Run W(AelvorSeed);
	...
	VAELEN_LOG_INFO(LogGate, "frozen: half=%016llx end=%016llx log=%016llx", static_cast<unsigned long long>(AtHalf),
					static_cast<unsigned long long>(AtEnd), static_cast<unsigned long long>(Log));
#if VAELEN_COLONYGATE_FROZEN_END != 0x0ull
	VT_CHECK_EQ(AtHalf, Hash64{VAELEN_COLONYGATE_FROZEN_HALF});
	VT_CHECK_EQ(AtEnd, Hash64{VAELEN_COLONYGATE_FROZEN_END});
	VT_CHECK_EQ(Log, Hash64{VAELEN_COLONYGATE_LOG});
#endif
}
