...
// Phase 12.08: the Phase 12 gate.
//
// A century at 256 with the mining colony at full detail, and this time the
// people in it have lives of their own: they speak and give and take, they
// think things of each other, they tell each other, the places around them come
// to carry names, and those names cost somebody their freedom. Every invariant
// of every phase from 04 to 12 checked each decade, the whole thing frozen, and
// a life played inside it replayed into a fresh world of the same seed.
//
// The harness is the Phase 11 gate's - the whole stack, which is what a gate
// needs - with the seven systems of Phase 12 added on top of it.
//
// ADR-0095 is why the volumes below are asserted before anything else is
// believed: a gate has to be FULL of what it claims to measure, and the first
// run of the Phase 11 gate recorded 1081 intents where it should have had
// 14400 and passed anyway.
//
// STATUS: PROTOTYPE (Phase 12) - the phase gate: long-duration, replay, frozen
...
// 300, sixty years of detail on the ground where the ore and the people both
// are, the colony founded, bound and made LIVELY there, and forty years lived a
// day at a time inside it - 14400 played intents over two lives, and 327267
// acts by everybody else.
//
// The first run of this gate recorded these same digests with Phase 12 measuring
// zero, because the colony's ground was never marked lively and LivingSystem had
...
namespace
{
	...
	/// What a colony's people do with their days, at the gate's scale.
	///
	/// 12.01's own rate is 200 in a thousand, which is right for one region of
	/// 1428 being looked at closely. A colony of four thousand acting at that
	/// rate for forty years is eleven million acts, and a gate that takes two
	/// hours is a gate nobody runs. Twenty in a thousand keeps every system of
	/// the phase working on real volume - the numbers are asserted below rather
	/// than hoped for - without the gate becoming the slowest thing in the
	/// project. The rate is the only rule of the phase this gate moves.
	LivingRules GateLiving()
	{
		LivingRules R;
		R.ActPerMille = 20;
		return R;
	...
	struct Run
	{
		explicit Run(uint64 Seed, BuildingRules InBuilds = BuildingRules{}, WorksRules InWorth = WorksRules{},
					 ...
					 SiegeRules InWalls = SiegeRules{}, LivingRules InLive = GateLiving(),
					 JudgementRules InJudge = JudgementRules{})
			: Instance(Config(Seed)), Ages(Instance, PreHistoryRules{})
		{
			...
			// Phase 12: the people of the colony live lives of their own.
			...
			Annals = ColonyChronicleTypes::Declare(Instance);
			...
			// Phase 11: mined ground reaps with whoever it has not bound (11.06),
			// and its ore is lifted at the day by the colony rather than by the year.
			Harvest->ObserveMined(Pit.Mined);
			Harvest->ObserveBonds(Bondage.Bond);
			Orgs->ObserveBonds(Bondage.Bond);
			Rock =
				std::make_unique<MiningSystem>(Instance, Ages.Types(), Persons, Families, Economy, Pit, MiningRules{});
			...
			// Phase 12, on top of everything. Living daily after the player's own
			// day so an unplayed person and a played one act in the same order
			// every tick; Repute after Living; Fame and Judgement yearly, after
			// the year's trade and the year's bondage.
			Lives_ = std::make_unique<LivingSystem>(Instance, Ages.Types(), Persons, Live, InLive);
			Lives_->ObserveDoing(Hands.get());
			Lives_->ObserveTraits(Traits.Traits);
			Lives_->RunAfter("PlayerOrders");
			Gossip = std::make_unique<ReputeSystem>(Instance, Persons, Names, ReputeRules{});
			Gossip->RunAfter("Living");
			Abroad = std::make_unique<FameSystem>(Instance, Ages.Types(), Persons, Names, Trade, Fame_, FameRules{});
...
namespace
{
	/// The ore-richest region of a world: the ground a mining colony stands on.
	/// 10.02 already knew this is not the ground the most people live on, and at
	/// 256 regions it is still not.
	uint32 Orerichest(const Run& W)
	{
		...
		// Peopled, and not merely rich. The richest ore in AELVOR is under ground
		// almost nobody lives on, and a colony of thirty-six is not the colony
		// this phase is about: the first run of this gate founded one there and
		// spent ninety years measuring a hamlet.
		uint32 Best = 0;
		uint64 Most = 0;
			...
			W.Instance.Components()
				...
				.ForEach(
					[&](EntityHandle H, const RegionInfo& Info)
					{
						...
						People = P != nullptr ? P->Total : 0u;
					});
			// Ore under it AND people on it: the product ranks a place that is
			// both, and a place that is only one of them falls away.
			const uint64 Worth = uint64{Ore[R]} * People;
			if (Worth > Most)
			{
				Most = Worth;
				Best = R;
	...
	/// Every invariant the colony adds to the ones below it.
	uint32 CheckColonyInvariants(VaelenTest::Context& Ctx, const Run& W, uint32 Region, uint32 Year)
	{
		uint32 Failures = 0;
		const MiningStats M = MeasureMining(W.Instance, W.Ages.Types(), W.Pit, Region);
		const HoldingStats H = MeasureHolding(W.Instance, W.Persons, W.Bondage, W.Standing, BondageRules{}, Region);
		// Written out rather than run through a helper: VT_CHECK_MSG wants a
		// literal, so a lambda taking the message would be a format-security
		// error, and the messages are worth reading in place anyway.
		Failures += M.Colonies == 1 ? 0u : 1u;
		VT_CHECK_MSG(M.Colonies == 1, "there is one colony, every decade of the century");
		Failures += M.Taken <= M.Richness ? 0u : 1u;
		VT_CHECK_MSG(M.Taken <= M.Richness, "no seam ever gave more than it held");
		Failures += M.WorkedOut <= M.Seams ? 0u : 1u;
		VT_CHECK_MSG(M.WorkedOut <= M.Seams, "no more seams are spent than the ground has");
		Failures += H.ByPerson + H.ByRegion == H.Bound ? 0u : 1u;
		VT_CHECK_MSG(H.ByPerson + H.ByRegion == H.Bound, "everybody bound is held by somebody or by the colony");
		Failures += H.Fullest <= BondageRules{}.MaxHeldPerHolder ? 0u : 1u;
		VT_CHECK_MSG(H.Fullest <= BondageRules{}.MaxHeldPerHolder, "and no holder takes more than 05.04 allows");
		Failures += H.Bound <= H.People ? 0u : 1u;
		VT_CHECK_MSG(H.Bound <= H.People, "nobody is bound who is not there");
		(void)Year;
		return Failures;
	}

	/// The invariants of Phase 12: what people believe, what places say, and
	/// what is done about it.
	uint32 CheckBeliefInvariants(VaelenTest::Context& Ctx, const Run& W, uint32 Year)
	{
		uint32 Failures = 0;
		const ReputeStats R = MeasureRepute(W.Instance, W.Persons, W.Names);
		const FameStats F = MeasureFame(W.Instance, W.Fame_);
		const JudgementStats J = MeasureJudgement(W.Instance, W.Persons, W.Bondage);

		// NOT "Opinions >= Heard", which the first run of this gate failed at
		// year 70 and was right to. Heard is a running total of everything ever
		// told about somebody; Opinions is how many of the eight slots are in
		// use right now. A person told about a hundred times holds eight
		// opinions, and the hundred is not an error.
		Failures += R.Opinions <= R.ThoughtOf * MostThoughtOf ? 0u : 1u;
		VT_CHECK_MSG(R.Opinions <= R.ThoughtOf * MostThoughtOf,
					 "nobody is thought about by more people than the handful allows");
		Failures += R.Best <= ReputeRules{}.Most ? 0u : 1u;
		VT_CHECK_MSG(R.Best <= ReputeRules{}.Most, "nobody is thought better of than the scale allows");
		Failures += R.Worst >= ReputeRules{}.Least ? 0u : 1u;
		VT_CHECK_MSG(R.Worst >= ReputeRules{}.Least, "nor worse of");
		Failures += F.Abroad <= F.Names ? 0u : 1u;
		VT_CHECK_MSG(F.Abroad <= F.Names, "no more names came from abroad than are said at all");
		Failures += F.Furthest <= FameRules{}.MostHops ? 0u : 1u;
		VT_CHECK_MSG(F.Furthest <= FameRules{}.MostHops, "no name went further than the world lets it");
		Failures += F.Names <= F.Places * MostNames ? 0u : 1u;
		VT_CHECK_MSG(F.Names <= F.Places * MostNames, "no place carries more names than it has room for");
		// NOT "Pardoned <= Condemned", which the first run failed and was also
		// right to. A pardon frees anybody the place thinks well enough of, and
		// most bound people in a colony were bound by 05.04's debt or by 11.04's
		// founding rather than by judgement. A good name buying somebody out of
		// a debt bondage they were born into is the system working, not a leak.
		Failures += J.BoundNow <= J.Condemned ? 0u : 1u;
		VT_CHECK_MSG(J.BoundNow <= J.Condemned, "nobody is bound by a judgement that was never passed");
		(void)Year;
		return Failures;
	}
} // namespace

VAELEN_TEST(GameplayGate, ACenturyOfALivedColonyHoldsEveryInvariantAndReplays)
{
	Run W(AelvorSeed);
	VT_REQUIRE(W.Ages.Generate(Run::Square(256), 300));
	const uint32 Where = Orerichest(W);
	VT_REQUIRE(Where != 0);
	// Held from this tick and not from the beginning of time: LodRules::Held
	// applies through the whole of the pre-history above, and 11.05 found that
	// empties the very region it is meant to keep.
	VT_CHECK(RequestDetail(W.Instance, W.Lod, Where));
	W.Ages.Run(BeforeYears);

	// The colony begins here, and its founding is an INPUT to the world exactly
	// as a taking is: the replay below does it on the same tick or it is
	// rebuilding a different world.
	const uint64 Founded = W.Instance.Now();
	VT_REQUIRE(FoundColony(W.Instance, W.Ages.Types(), W.Pit, Where));
	const uint32 Bound =
		BindColony(W.Instance, W.Ages.Types(), W.Persons, W.Bondage, W.Standing, W.Pit, Where, Founded);
	VT_REQUIRE(Bound > 0);
	const uint32 Overseers = RaiseOverseers(W.Instance, W.Ages.Types(), W.Organizations, W.Pit, Where, 24u, Founded);
	VT_CHECK(Overseers != 0);
	// And its people live lives of their own from the same tick. An INPUT like
	// the founding, so the replay does it too - and the reason the first run of
	// this gate was hollow: without it LivingSystem has no ground to walk and
	// the whole of Phase 12 measured zero while every check it made was true.
	VT_REQUIRE(MakeLively(W.Instance, W.Ages.Types(), W.Live, Where));
	const uint32 Seam = SeamLeft(W.Instance, W.Ages.Types(), W.Pit, Where);
	VAELEN_LOG_INFO(LogGate, "colony founded in region %u at year %u: %u bound, %u in the seams", Where,
					static_cast<uint32>(Founded / TicksPerYear), Bound, Seam);

	// A life inside it. The start of 10.02 prefers ground with ore under it, and
	// the colony stands on the ore-richest ground there is, so the world hands
	// back somebody of the colony without being told to - which is 11.07's
	// "the player's place in it", and it is checked rather than assumed.
	StartRules OnTheOre;
	OnTheOre.ToAge = 25;
	// Whoever the colony has, bound or not. A colony's bondage erodes - 11.06
	// measured 1116 becoming 247 over sixty years, and at this size it reaches
	// zero inside a decade - so a start that insists on a bound person finds
	// nobody after the first life and the gate spends ninety years with nobody
	// played. That is the hollow gate ADR-0089 was written about, and the first
	// run of this one was exactly it: 1081 intents where there should be 14400.
	OnTheOre.WantBound = 0;
	const uint32 Who = W.Begin(OnTheOre);
	VT_REQUIRE(Who != 0);
	const PersonInfo* Started = W.Person_(Who);
	VT_REQUIRE(Started != nullptr);
	VT_CHECK_MSG(Started->Region == Where, "the life the world offered stands on the colony's ground");
	VT_REQUIRE(W.Open());
	...
	for (uint32 Year = 1; Year <= LifeYears; ++Year)
	{
		for (uint32 Day = 0; Day < DaysPerYear; ++Day)
		{
			...
			if (Playing != 0 && (Now_ == nullptr || Now_->State != static_cast<uint8>(LifeState::Alive)))
			{
				...
				const uint32 Next_ = W.Begin(OnTheOre);
				Playing = Next_;
				if (Next_ != 0)
				{
		...
		if (Year % 10 == 0)
		{
			...
			Failures += CheckColonyInvariants(Ctx, W, Where, BeforeYears + Year);
			Failures += CheckBeliefInvariants(Ctx, W, BeforeYears + Year);
			const MiningStats M = MeasureMining(W.Instance, W.Ages.Types(), W.Pit, Where);
			const HoldingStats H = MeasureHolding(W.Instance, W.Persons, W.Bondage, W.Standing, BondageRules{}, Where);
			const ReputeStats Op = MeasureRepute(W.Instance, W.Persons, W.Names);
			const FameStats Fa = MeasureFame(W.Instance, W.Fame_);
			const JudgementStats Ju = MeasureJudgement(W.Instance, W.Persons, W.Bondage);
			VAELEN_LOG_INFO(LogGate, "year %u: %u in the colony, %u bound (%u by it), %u of %u lifted, %u seams spent",
							BeforeYears + Year, H.People, H.Bound, H.ByRegion, M.Taken, M.Richness, M.WorkedOut);
			VAELEN_LOG_INFO(LogGate,
							"          %u thought of (%u heard), best %d worst %d; %u names in %u places (%u abroad, "
							"%u roads); %u condemned, %u still bound",
							Op.ThoughtOf, Op.Heard, Op.Best, Op.Worst, Fa.Names, Fa.Places, Fa.Abroad, Fa.Furthest,
							Ju.Condemned, Ju.BoundNow);
		}
		if (Year == LifeYears / 2)
		{
	...
	// ADR-0095, before any verdict: a gate has to be FULL of what it claims to
	// measure. The Phase 11 gate passed its first run on 1081 intents where it
	// should have had 14400, because the played person kept dying with nobody
	// to replace them, and every check it made was true of a corpse. So the
	// volumes are read out loud and asserted here, and what follows is only
	// believed because they hold.
	const DoingStats Verbs = MeasureDoings(W.Instance);
	const ReputeStats Op = MeasureRepute(W.Instance, W.Persons, W.Names);
	const FameStats Fa = MeasureFame(W.Instance, W.Fame_);
	const JudgementStats Ju = MeasureJudgement(W.Instance, W.Persons, W.Bondage);
	const GameplayContext Heard{W.Persons, W.Names, W.Papers, W.Charts, W.Fame_};
	const BeliefStats Ch = MeasureBelief(W.Instance, W.Ages.Types(), Heard);
	VAELEN_LOG_INFO(LogGate, "the volume: %zu played intent(s) over %u life/lives; %u acts in all, %u given, %u taken",
					Stream.size(), Lives, Verbs.Acts, Verbs.Gave, Verbs.Took);
	VAELEN_LOG_INFO(LogGate,
					"            %u people thought of, %u opinions (%u heard), best %d worst %d; %u names in %u "
					"places, %u of them abroad, %u roads at the furthest",
					Op.ThoughtOf, Op.Opinions, Op.Heard, Op.Best, Op.Worst, Fa.Names, Fa.Places, Fa.Abroad,
					Fa.Furthest);
	VAELEN_LOG_INFO(LogGate,
					"            %u condemned, %u pardoned, %u still bound; %u events of belief, %u with a "
					"sentence, %u judgements on a telling",
					Ju.Condemned, Ju.Pardoned, Ju.BoundNow, Ch.Events, Ch.Described, Ch.Believed);
	VT_CHECK_MSG(Stream.size() == static_cast<usize>(LifeYears) * DaysPerYear,
				 "a life was played on every single day of the forty years, which is what FULL means");
	VT_CHECK_MSG(Verbs.Took > 0, "somebody took something from somebody, so a bad name can exist at all");
	VT_CHECK_MSG(Op.ThoughtOf > 0, "people think things of each other");
	VT_CHECK_MSG(Op.Heard > 0, "and some of it reached somebody it never happened to");
	VT_CHECK_MSG(Op.Worst < 0, "and the world thinks ill of somebody, not only well");
	VT_CHECK_MSG(Fa.Names > 0, "places carry names");
	VT_CHECK_MSG(Fa.Abroad > 0, "and some of those names crossed a road to get there");
	VT_CHECK_MSG(Ju.Condemned > 0, "a name cost somebody their freedom");
	VT_CHECK_MSG(Ch.Described == Ch.Events, "and every act of belief in a century has a sentence");
	VAELEN_LOG_INFO(LogGate, "a century in %.1f s: %u life(s), %zu intent(s); end=%016llx log=%016llx", Seconds(Start),
					Lives, Stream.size(), static_cast<unsigned long long>(AtEnd), static_cast<unsigned long long>(Log));

	// The replay. A fresh world of the same seed, given the founding, the
	// takings and the intents on the ticks they happened, and nothing else.
	Run R(AelvorSeed);
	VT_REQUIRE(R.Ages.Generate(Run::Square(256), 300));
	VT_CHECK_EQ(Orerichest(R), Where);
	VT_CHECK(RequestDetail(R.Instance, R.Lod, Where));
	R.Ages.Run(BeforeYears);
	VT_CHECK_EQ(R.Instance.Now(), Founded);
	VT_CHECK(FoundColony(R.Instance, R.Ages.Types(), R.Pit, Where));
	VT_CHECK_EQ(BindColony(R.Instance, R.Ages.Types(), R.Persons, R.Bondage, R.Standing, R.Pit, Where, Founded), Bound);
	VT_CHECK(RaiseOverseers(R.Instance, R.Ages.Types(), R.Organizations, R.Pit, Where, 24u, Founded) == Overseers);
	VT_CHECK(MakeLively(R.Instance, R.Ages.Types(), R.Live, Where));
	...
	auto TakeUp = [&](Run& In)
	{
		while (Took < Takings.size() && Takings[Took].Tick <= In.Instance.Now())
		{
			...
			Wrong += In.Begin(OnTheOre) == Takings[Took].Person ? 0u : 1u;
			VT_CHECK(In.Open());
			++Took;
		}
	};
	...
	VAELEN_LOG_INFO(LogGate, "replay: %zu of %zu intent(s), %zu of %zu taking(s), %u answered differently", Next,
					Stream.size(), Took, Takings.size(), Wrong);
	...
	VT_CHECK_EQ(MeasureMining(R.Instance, R.Ages.Types(), R.Pit, Where).Digest,
				MeasureMining(W.Instance, W.Ages.Types(), W.Pit, Where).Digest);
	// And the phase's own state, which the world digest already covers but which
	// is worth naming separately: if belief ever replayed differently, this is
	// the line that would say so rather than a single opaque hash.
	const GameplayContext HeardAgain{R.Persons, R.Names, R.Papers, R.Charts, R.Fame_};
	VT_CHECK_EQ(MeasureFame(R.Instance, R.Fame_).Digest, Fa.Digest);
	VT_CHECK_EQ(MeasureRepute(R.Instance, R.Persons, R.Names).Digest, Op.Digest);
	VT_CHECK_EQ(MeasureJudgement(R.Instance, R.Persons, R.Bondage).Digest, Ju.Digest);
	VT_CHECK_EQ(MeasureBelief(R.Instance, R.Ages.Types(), HeardAgain).Digest, Ch.Digest);
	// Frozen. Recorded on gcc 13 / Linux x86_64 on 2026-09-09: AELVOR 256 at year
	// 300, sixty years of detail, the colony founded where the ore and the people
	// both are, and forty years lived a day at a time inside it.
	VAELEN_LOG_INFO(LogGate, "frozen: half=%016llx end=%016llx log=%016llx belief=%016llx",
					static_cast<unsigned long long>(AtHalf), static_cast<unsigned long long>(AtEnd),
					static_cast<unsigned long long>(Log), static_cast<unsigned long long>(Ch.Digest));
#if VAELEN_GAMEPLAYGATE_FROZEN_END != 0x0ull
	VT_CHECK_EQ(AtHalf, Hash64{VAELEN_GAMEPLAYGATE_FROZEN_HALF});
	VT_CHECK_EQ(AtEnd, Hash64{VAELEN_GAMEPLAYGATE_FROZEN_END});
	VT_CHECK_EQ(Log, Hash64{VAELEN_GAMEPLAYGATE_LOG});
	VT_CHECK_EQ(Ch.Digest, Hash64{VAELEN_GAMEPLAYGATE_BELIEF});
#endif
}
