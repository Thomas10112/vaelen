...
// Phase 09.03: settlements as places - a settlement of 06.04 is a fact about
...
// Recorded on clang 18 / Linux x86_64 on 2026-09-08 (09.03): AELVOR 128 at
// year 300, the two most peopled regions detailed, 120 years with every Phase
// 04 to 09 system.
#define VAELEN_PLACES_FROZEN_128 0xc61d14222c153d7bull

namespace
{
	VAELEN_DEFINE_LOG_CATEGORY(LogPlaces);
	...
	struct Run
	{
		explicit Run(uint64 Seed, BuildingRules InBuilds = BuildingRules{}, WorksRules InWorth = WorksRules{},
					 PlaceRules InTowns = PlaceRules{}, ArmyRules InHosts = ArmyRules{},
...
VAELEN_TEST(Places, ASettlementGetsABody)
{
	Run W(AelvorSeed);
	VT_REQUIRE(Settled(W, 120));
	const PlaceStats S = W.Towns_();
	VAELEN_LOG_INFO(LogPlaces, "places=%u standing=%u emptied=%u townsfolk=%u largest=%u works=%u loose=%u bad=%u",
					S.Places, S.Standing, S.Emptied, S.Townsfolk, S.Largest, S.Works, S.Loose, S.Bad);
	VT_CHECK_MSG(S.Bad == 0, "every place must keep its invariants");
	VT_CHECK_MSG(S.Places > 0, "a world that trades must have put settlements on the map");
	VT_CHECK(S.Standing > 0);
	VT_CHECK_MSG(S.Largest >= 1 && S.Largest <= PlaceRules{}.MostSize, "no place outgrows its cap");

	// Every standing place is on a tile of its own region, and no two share one.
	std::vector<uint32> Tiles;
	uint32 Seen = 0;
	for (const uint32 Region : W.Ranked())
	{
		const PlaceInfo* Town = W.TownIn(Region);
		if (Town == nullptr)
		{
			continue;
		}
		++Seen;
		VT_CHECK_EQ(W.RegionOfTile(Town->Tile), Region);
		VT_CHECK(Town->Grown >= Town->Size);
		Tiles.push_back(Town->Tile);
	}
	VT_CHECK(Seen > 0);
	std::sort(Tiles.begin(), Tiles.end());
	VT_CHECK_MSG(std::adjacent_find(Tiles.begin(), Tiles.end()) == Tiles.end(), "one place to a tile");

	// A body belongs to a settlement of 06.04 and stands where it says.
	uint32 Bodies = 0;
	W.Instance.Components()
		.GetPool(W.Trade.Settlement)
		.ForEach(
			[&](EntityHandle, const SettlementInfo& T)
			{
				const PlaceInfo* Town = W.Place(T.Index);
				if (Town == nullptr)
				{
					return;
				}
				++Bodies;
				VT_CHECK_EQ(Town->Region, T.Region);
				VT_CHECK_EQ(Town->Size == 0, T.Abandoned != 0);
			});
	VT_CHECK_EQ(Bodies, S.Places);
}

VAELEN_TEST(Places, AWorkStandsInTheTownWhereThereIsOne)
{
	Run W(AelvorSeed);
	VT_REQUIRE(Settled(W, 120));
	...
	VT_CHECK_MSG(S.Works + S.Loose == B.Standing, "every standing building stands somewhere");
	VT_CHECK_MSG(S.Works > 0, "a region with a town keeps its works in it");

	// A building is in its region's town when there is one, and loose when there
	// is not - and the town's own count agrees with the buildings on it.
	uint32 Checked = 0;
	W.Instance.Components()
		.GetPool(W.Builds.Building)
		.ForEach(
			[&](EntityHandle, const BuildingInfo& Work)
			{
				const BuildingPlace* At = W.Stands(Work.Index);
				if (At == nullptr)
				{
					return;
				}
				++Checked;
				const PlaceInfo* Town = W.TownIn(Work.Region);
				const bool HasTown = Town != nullptr && Town->Size != 0;
				VT_CHECK_EQ(At->Settlement != 0, HasTown);
				if (HasTown)
				{
					VT_CHECK_EQ(At->Settlement, Town->Settlement);
					VT_CHECK_EQ(At->Tile, Town->Tile);
					VT_CHECK_EQ(W.RegionOfTile(At->Tile), Work.Region);
				}
			});
	VT_CHECK_EQ(Checked, B.Standing + B.Ruined);
}

VAELEN_TEST(Places, RulesAndEdges)
{
	// Works kept out of the towns stand loose, every one of them.
	{
		PlaceRules Country;
		Country.WorksInTown = 0;
		Run W(AelvorSeed, BuildingRules{}, WorksRules{}, Country);
		VT_REQUIRE(Settled(W, 80));
		const PlaceStats S = W.Towns_(Country);
		VT_CHECK_EQ(S.Works, 0u);
		VT_CHECK_EQ(S.Loose, W.Built().Standing);
		VT_CHECK_EQ(S.Bad, 0u);
	}
	// A cap of one is a cap of one, however many people and however much traffic.
	{
		PlaceRules Small;
		Small.MostSize = 1;
		Run W(AelvorSeed, BuildingRules{}, WorksRules{}, Small);
		VT_REQUIRE(Settled(W, 80));
		const PlaceStats S = W.Towns_(Small);
		VT_CHECK_EQ(S.Largest, S.Standing > 0 ? 1u : 0u);
		VT_CHECK_EQ(S.Bad, 0u);
	}
	// Nothing is known about what was never settled.
	{
		Run W(AelvorSeed);
		VT_REQUIRE(W.Ages.Generate(Run::Square(64), 60));
		VT_CHECK(W.Place(0) == nullptr);
		VT_CHECK(W.Place(999999) == nullptr);
		VT_CHECK(W.TownIn(0) == nullptr);
		VT_CHECK(W.TownIn(999999) == nullptr);
		VT_CHECK(W.Stands(0) == nullptr);
		VT_CHECK(W.Stands(999999) == nullptr);
		VT_CHECK_EQ(W.RegionOfTile(4000000000u), 0u);
		// A small young world may already trade; whatever it has, it adds up.
		const PlaceStats S = W.Towns_();
		VT_CHECK_EQ(S.Bad, 0u);
		VT_CHECK_EQ(S.Places, S.Standing + S.Emptied);
	}
}

VAELEN_TEST(Places, DeterministicSnapshotSafeAndFrozen)
{
	Run A(AelvorSeed);
	Run B(AelvorSeed);
	VT_REQUIRE(Settled(A, 120));
	VT_REQUIRE(Settled(B, 120));
	const PlaceStats SA = A.Towns_();
	VT_CHECK_EQ(SA.Digest, B.Towns_().Digest);
	...
	VT_CHECK_EQ(C.Towns_().Digest, SA.Digest);
	...
	VT_CHECK_EQ(C.Towns_().Digest, A.Towns_().Digest);

	VAELEN_LOG_INFO(LogPlaces, "places digest=%016llx", static_cast<unsigned long long>(SA.Digest));
#if VAELEN_PLACES_FROZEN_128 != 0x0ull
	VT_CHECK_EQ(SA.Digest, Hash64{VAELEN_PLACES_FROZEN_128});
#endif
}
