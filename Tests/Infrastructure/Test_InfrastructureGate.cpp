...
#define VAELEN_WORKSGATE_FROZEN_256_250 0xbe1201b8ec11ae9cull
#define VAELEN_WORKSGATE_FROZEN_256_500 0xff95117c4902b1dcull
#define VAELEN_WORKSGATE_LOG_256_500 0x603c796f7a6ca7c5ull
#define VAELEN_WORKSGATE_TEXT_256_500 0xfd977cdd5ed25e27ull

namespace
{
	VAELEN_DEFINE_LOG_CATEGORY(LogWorksGate);
...
VAELEN_TEST(InfrastructureGate, FiveCenturiesOfBuildingAt256HoldEveryInvariantAndFreeze)
{
	Run W(AelvorSeed, BuildingRules{}, WorksRules{}, PlaceRules{}, RoadRules{}, DecayRules{}, LogisticsRules{},
		  WorksChronicleRules{}, ArmyRules{}, DiplomacyRules{}, FactionRules{}, SuccessionRules{}, WideReach());
	VT_REQUIRE(W.Ages.Generate(Run::Square(256), 300));
	// Two powers, so that the whole of the phase beneath this one runs too: a
	// world at peace never puts a host on ground anybody built on.
	...
	for (uint32 Decade = 1; Decade <= 50; ++Decade)
	{
		...
		if (Decade % 10 == 0)
		{
			...
			const DecayStats De = W.Wear();
			const LogisticsStats Lo = W.Lanes_();
			VAELEN_LOG_INFO(LogWorksGate,
							"year %u: %u work(s) standing (%u sound), %u ruin(s), %u town(s), %u road(s) made, "
							"%u region(s) served, %u record(s)",
							Decade * 10, Bu.Standing, De.Sound, De.Fallen, Pl.Standing, Ro.Made, Lo.Served,
							W.Annals_().Records);
		}
		if (Decade == 25)
		{
	...
	// Five centuries of building really happened: things raised, made bigger,
	// fallen in and built back, towns settled and emptied, roads cut and lost,
	// and ground that is easier to cross than it was.
	const BuildingStats Bu = W.Built();
	const PlaceStats Pl = W.Towns_();
	const RoadStats Ro = W.Ways_();
	const DecayStats De = W.Wear();
	const LogisticsStats Lo = W.Lanes_();
	const WorksChronicleStats An = W.Annals_();
	VT_CHECK(Bu.Raised > 0 && Bu.Enlarged > 0);
	VT_CHECK(De.Fallen > 0 && De.Rebuilt > 0);
	VT_CHECK(Pl.Standing > 0 && Pl.Emptied > 0);
	VT_CHECK(Ro.Cuttings > 0 && Ro.Losses > 0);
	VT_CHECK(Lo.Served > 0);
	VT_CHECK(An.Records > 0 && An.Described == An.Records);
	VT_CHECK(Text.find(" raised a granary.") != std::string::npos);
	VT_CHECK(Text.find(" fell in.") != std::string::npos);
	VAELEN_LOG_INFO(LogWorksGate,
					"five centuries: %u raised, %u enlarged, %u fallen, %u rebuilt, %u town(s), %u road(s) cut, "
					"%u lost, %u region(s) served, %u record(s)",
					Bu.Raised, Bu.Enlarged, De.Fallen, De.Rebuilt, Pl.Standing, Ro.Cuttings, Ro.Losses, Lo.Served,
					An.Records);

	// The world of year 250 is still there, and it runs on into the same year
	// 500 the first one reached.
	VT_REQUIRE(!Image.empty());
	Run R(AelvorSeed, BuildingRules{}, WorksRules{}, PlaceRules{}, RoadRules{}, DecayRules{}, LogisticsRules{},
		  WorksChronicleRules{}, ArmyRules{}, DiplomacyRules{}, FactionRules{}, SuccessionRules{}, WideReach());
...
#if VAELEN_WORKSGATE_FROZEN_256_250 != 0x0ull
	VT_CHECK_EQ(At250, Hash64{VAELEN_WORKSGATE_FROZEN_256_250});
	VT_CHECK_EQ(At500, Hash64{VAELEN_WORKSGATE_FROZEN_256_500});
	VT_CHECK_EQ(Log, Hash64{VAELEN_WORKSGATE_LOG_256_500});
	VT_CHECK_EQ(TextDigest, Hash64{VAELEN_WORKSGATE_TEXT_256_500});
#endif
}

VAELEN_TEST(InfrastructureGate, AHostOnTheGroundWearsWhatStandsOnIt)
{
	// 09.05 wires a foreign host into the same wear a flood is, and its own
	// suite never put one on ground with works. This is where that is settled.
	Run W(AelvorSeed, BuildingRules{}, WorksRules{}, PlaceRules{}, RoadRules{}, DecayRules{}, LogisticsRules{},
		  WorksChronicleRules{}, ArmyRules{}, DiplomacyRules{}, FactionRules{}, SuccessionRules{}, WideReach());
	...
	std::pair<uint32, uint32> Under{0u, 0u};
	// The snapshot is taken AFTER the host is seen standing there, not before:
	// Decay runs early in a tick and Marching late, so a host observed at the
	// end of a year is where the next year's decay will find it.
	for (uint32 Year = 0; Year < 300 && Under.first == 0; ++Year)
	{
		...
		W.Ages.Run(1);
		Under = W.ForeignHostOnWorks();
	}
	if (Under.first != 0)
	{
		SaveSnapshot(W.Instance, Before);
	}
	// A world where no host ever stood on ground somebody had built on would
	// prove nothing either way.
	VT_REQUIRE(Under.first != 0);
	...
	DecayRules Savage;
	Savage.WarWear = 600;
	Savage.SiegeWear = 600;
	Run Quiet(AelvorSeed, BuildingRules{}, WorksRules{}, PlaceRules{}, RoadRules{}, Calm, LogisticsRules{},
			  WorksChronicleRules{}, ArmyRules{}, DiplomacyRules{}, FactionRules{}, SuccessionRules{}, WideReach());
	Run Hard(AelvorSeed, BuildingRules{}, WorksRules{}, PlaceRules{}, RoadRules{}, Savage, LogisticsRules{},
			 WorksChronicleRules{}, ArmyRules{}, DiplomacyRules{}, FactionRules{}, SuccessionRules{}, WideReach());
	...
	const uint32 Stood = Quiet.Repair(Under.first, Under.second);
	VT_REQUIRE(Stood > 0);
	for (const uint32 P : Quiet.Powers())
	{
		Quiet.Endow(P, 60000);
	}
	for (const uint32 P : Hard.Powers())
	{
		Hard.Endow(P, 60000);
	}
	Quiet.Ages.Run(1);
	Hard.Ages.Run(1);
	const uint32 Sheltered = Quiet.Repair(Under.first, Under.second);
	const uint32 Trampled = Hard.Repair(Under.first, Under.second);
	VAELEN_LOG_INFO(LogWorksGate, "region %u work %u: %u -> %u with no host worth fearing, %u with one", Under.first,
					Under.second, Stood, Sheltered, Trampled);
	VT_CHECK_MSG(Trampled < Sheltered, "a foreign host on the ground takes more off what stands on it");
}
