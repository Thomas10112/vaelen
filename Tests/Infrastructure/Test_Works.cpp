...
// 06.02, the wall of 08.04. Nothing here is a number of its own: every kind of
...
// Recorded on clang 18 / Linux x86_64 on 2026-09-08 (09.02): AELVOR 128 at
// year 300, the two most peopled regions detailed, 120 years with every Phase
// 04 to 09 system.
#define VAELEN_WORTH_FROZEN_128 0xefa42015d78f5894ull

namespace
{
	VAELEN_DEFINE_LOG_CATEGORY(LogWorth);
...
namespace
{
	...
	/// more with a gift of goods every year so the common stocks are not the
	/// thing under test.
	...
	/// Every kind of work worth nothing: the same world, built the same way, for
	/// the same price, with none of it doing anything.
	WorksRules Worthless()
	{
		WorksRules Nil;
		Nil.GranaryPerSize = 0;
		Nil.MillPerSize = 0;
		Nil.SmithyPerSize = 0;
		Nil.WallPerSize = 0;
		return Nil;
...
VAELEN_TEST(Works, WhatStandsIsWrittenWhereItIsRead)
{
	Run W(AelvorSeed);
	VT_REQUIRE(Settled(W, 120));
	const BuildingStats B = W.Built();
	VT_REQUIRE(B.Standing > 0);
	VT_CHECK_EQ(B.Bad, 0u);

	const WorksStats S = W.Worth_();
	VAELEN_LOG_INFO(LogWorth, "granaries=%u fields=%u craft=%u walls=%u most=%u/%u/%u/%u bad=%u", S.Granaries, S.Fields,
					S.Craft, S.Walls, S.MostGranary, S.MostFields, S.MostCraft, S.MostWall, S.Bad);
	VT_CHECK_MSG(S.Bad == 0, "every number written must be the arithmetic of what stands there");
	VT_CHECK_MSG(S.Granaries > 0, "a world that built granaries must have written what they are worth");
	VT_CHECK_MSG(S.Fields > 0, "and what its mills are worth");

	// Nothing is written past its cap, and every number is the one WorthOf gives.
	const WorksRules R;
	VT_CHECK(S.MostGranary <= R.GranaryMost);
	VT_CHECK(S.MostFields <= R.MillMost);
	VT_CHECK(S.MostCraft <= R.SmithyMost);
	VT_CHECK(S.MostWall <= R.WallMost);
	uint32 Checked = 0;
	for (const uint32 Region : W.Ranked())
	{
		const RegionWorks* Held = W.Held(Region);
		if (Held == nullptr || Held->Standing == 0)
		{
			continue;
		}
		++Checked;
		VT_CHECK_EQ(W.Worth(Region, Work::Granary),
					std::min(R.GranaryMost, Held->Kept[static_cast<uint32>(Work::Granary)] * R.GranaryPerSize));
		VT_CHECK_EQ(W.Worth(Region, Work::Mill),
					std::min(R.MillMost, Held->Kept[static_cast<uint32>(Work::Mill)] * R.MillPerSize));
	}
	VT_CHECK(Checked > 0);
}

VAELEN_TEST(Works, AMillMeansMoreGrainAndASmithyMoreCloth)
{
	// Two worlds of one seed that build the same things for the same price. In
	// one of them the things do nothing.
	Run Rich(AelvorSeed);
	Run Bare(AelvorSeed, BuildingRules{}, Worthless());
	VT_REQUIRE(Settled(Rich, 120));
	VT_REQUIRE(Settled(Bare, 120));

	const BuildingStats A = Rich.Built();
	const BuildingStats B = Bare.Built();
	VT_REQUIRE(A.Standing > 0);
	VT_CHECK_MSG(A.Timber == B.Timber, "the same buildings cost the same in both worlds");

	const uint64 GrainRich = Rich.MadeEver(Good::Grain);
	const uint64 GrainBare = Bare.MadeEver(Good::Grain);
	VAELEN_LOG_INFO(LogWorth, "grain reaped: with mills %llu, without %llu", static_cast<unsigned long long>(GrainRich),
					static_cast<unsigned long long>(GrainBare));
	VT_CHECK_MSG(GrainRich > GrainBare, "a mill means the same fields and the same hands give more grain");

	const uint64 ClothRich = Rich.HeldEverywhere(Good::Cloth);
	const uint64 ClothBare = Bare.HeldEverywhere(Good::Cloth);
	VAELEN_LOG_INFO(LogWorth, "cloth held: with smithies %llu, without %llu",
					static_cast<unsigned long long>(ClothRich), static_cast<unsigned long long>(ClothBare));
	VT_CHECK_MSG(ClothRich >= ClothBare, "a smithy means more of what the craft makes");

	// And the world with worthless works wrote no worth at all.
	const WorksStats Nothing = Bare.Worth_(Worthless());
	VT_CHECK_EQ(Nothing.MostFields, 0u);
	VT_CHECK_EQ(Nothing.MostGranary, 0u);
	VT_CHECK_EQ(Nothing.Bad, 0u);
}

VAELEN_TEST(Works, AWallMeansASeatHoldsLonger)
{
	// One world run until a host is actually bringing a seat's wall down, and the
	// state of the year before it did, so that the same tick can be played twice.
	Run W(AelvorSeed, BuildingRules{}, WorksRules{}, ArmyRules{}, DiplomacyRules{}, FactionRules{}, SuccessionRules{},
		  WideReach());
	...
	for (uint32 Year = 0; Year < 200 && W.Walls_().Laid == 0; ++Year)
	{
		Push();
	}
	VT_REQUIRE(W.Walls_().Laid > 0);

	std::vector<uint8> Before;
	uint32 Breached = 0;
	for (uint32 Year = 0; Year < 80 && Breached == 0; ++Year)
	{
		Before.clear(); // SaveSnapshot appends to what it is given
		SaveSnapshot(W.Instance, Before);
		const std::vector<std::pair<uint32, uint32>> Was = W.Ramparts_();
		Push();
		const std::vector<std::pair<uint32, uint32>> Now = W.Ramparts_();
		for (const auto& [Region, Wall] : Now)
		{
			for (const auto& [Old, Stood] : Was)
			{
				// Only a besieger takes a wall down; mending and storming raise it.
				if (Old == Region && Wall < Stood && Breached == 0)
				{
					Breached = Region;
				}
			}
	...
	// A world where no host ever brought a wall down would prove nothing.
	VT_REQUIRE(Breached != 0);
	VT_REQUIRE(!Before.empty());

	// The same tick, from the same state, twice: once with nothing built and once
	// behind a wall. Only what the besieger brings down can differ.
	Run Open(AelvorSeed, BuildingRules{}, WorksRules{}, ArmyRules{}, DiplomacyRules{}, FactionRules{},
			 SuccessionRules{}, WideReach());
	Run Stone(AelvorSeed, BuildingRules{}, WorksRules{}, ArmyRules{}, DiplomacyRules{}, FactionRules{},
			  SuccessionRules{}, WideReach());
	VT_REQUIRE(LoadSnapshot(Open.Instance, Before.data(), Before.size()) == SnapshotResult::Ok);
	VT_REQUIRE(LoadSnapshot(Stone.Instance, Before.data(), Before.size()) == SnapshotResult::Ok);
	const SiegeInfo* Stood_ = Open.Rampart(Breached);
	VT_REQUIRE(Stood_ != nullptr);
	const uint32 Stood = Stood_->Wall;
	// One of them has a wall of stone before it, the other has nothing.
	Stone.RaiseWall(Breached, 5);
	Open.Ages.Run(1);
	Stone.Ages.Run(1);
	const SiegeInfo* Bare = Open.Rampart(Breached);
	const SiegeInfo* Built = Stone.Rampart(Breached);
	VT_REQUIRE(Bare != nullptr && Built != nullptr);
	VAELEN_LOG_INFO(LogWorth, "region %u wall %u -> open %u, walled %u", Breached, Stood, Bare->Wall, Built->Wall);
	VT_CHECK_MSG(Bare->Wall < Stood, "the open seat lost wall that year");
	VT_CHECK_MSG(Built->Wall > Bare->Wall, "a host before a built wall brings less of it down in a year");
	// And by exactly the factor the wall is worth: the same men, slowed.
	const uint32 Extra = Stone.WallExtra(Breached);
	VT_CHECK(Extra > 0);
	// And a smaller wall is worth less than a bigger one and more than none:
	// the same tick a third time, behind a wall of one instead of five.
	Run Thin(AelvorSeed, BuildingRules{}, WorksRules{}, ArmyRules{}, DiplomacyRules{}, FactionRules{},
			 SuccessionRules{}, WideReach());
	VT_REQUIRE(LoadSnapshot(Thin.Instance, Before.data(), Before.size()) == SnapshotResult::Ok);
	Thin.RaiseWall(Breached, 1);
	Thin.Ages.Run(1);
	const SiegeInfo* Small = Thin.Rampart(Breached);
	VT_REQUIRE(Small != nullptr);
	VAELEN_LOG_INFO(LogWorth, "wall worth %u per mille; down %u walled, %u thinly walled, %u open", Extra,
					Stood - Built->Wall, Stood - Small->Wall, Stood - Bare->Wall);
	VT_CHECK(Extra > Thin.WallExtra(Breached));
	VT_CHECK_MSG(Stood - Built->Wall <= Stood - Small->Wall, "a bigger wall never comes down faster");
	VT_CHECK_MSG(Stood - Small->Wall <= Stood - Bare->Wall, "and any wall never comes down faster than none");
}

VAELEN_TEST(Works, RulesAndEdges)
{
	Run W(AelvorSeed);
	VT_REQUIRE(Settled(W, 120));

	// A cap is a cap: worth a thousand per size, capped at ten, gives ten.
	WorksRules Tight;
	Tight.GranaryPerSize = 1000000u;
	Tight.GranaryMost = 10;
	Tight.MillPerSize = 1000000u;
	Tight.MillMost = 7;
	for (const uint32 Region : W.Ranked())
	{
		const RegionWorks* Held = W.Held(Region);
		if (Held == nullptr || Held->Kept[static_cast<uint32>(Work::Granary)] == 0)
		{
			continue;
		}
		VT_CHECK_EQ(W.Worth(Region, Work::Granary, Tight), 10u);
		break;
	}

	// Nothing is worth anything where there is nothing.
	VT_CHECK_EQ(W.Worth(0, Work::Granary), 0u);
	VT_CHECK_EQ(W.Worth(999999, Work::Mill), 0u);
	VT_CHECK_EQ(W.Worth(1, Work::Count), 0u);

	// A world that never built anything writes nothing, and that is coherent.
	Run Empty(AelvorSeed);
	VT_REQUIRE(Empty.Ages.Generate(Run::Square(64), 60));
	const WorksStats None = Empty.Worth_();
	VT_CHECK_EQ(None.Granaries, 0u);
	VT_CHECK_EQ(None.Fields, 0u);
	VT_CHECK_EQ(None.Walls, 0u);
	VT_CHECK_EQ(None.Bad, 0u);
}

VAELEN_TEST(Works, DeterministicSnapshotSafeAndFrozen)
{
	Run A(AelvorSeed);
	Run B(AelvorSeed);
	VT_REQUIRE(Settled(A, 120));
	VT_REQUIRE(Settled(B, 120));
	const WorksStats SA = A.Worth_();
	VT_CHECK_EQ(SA.Digest, B.Worth_().Digest);
	...
	VT_CHECK_EQ(C.Worth_().Digest, SA.Digest);
	...
	VT_CHECK_EQ(C.Worth_().Digest, A.Worth_().Digest);

	VAELEN_LOG_INFO(LogWorth, "worth digest=%016llx", static_cast<unsigned long long>(SA.Digest));
#if VAELEN_WORTH_FROZEN_128 != 0x0ull
	VT_CHECK_EQ(SA.Digest, Hash64{VAELEN_WORTH_FROZEN_128});
#endif
}
