...
// Recorded on clang 18 / Linux x86_64 (08.02): AELVOR 128 at year 300, the two
// most peopled regions detailed, 100 years with every Phase 04 to 08 system so
// far, marching included.
#define VAELEN_MARCH_FROZEN_128 0xae71515bd66bae56ull
#define VAELEN_MARCH_WALKED_128 16u
#define VAELEN_MARCH_MARCHES_128 9u
...
VAELEN_TEST(March, AHostEatsOffTheGroundItStandsOn)
{
	...
	if (A->Disbanded == 0)
	{
		...
		VT_CHECK(F->Taken > 0);
		VT_CHECK(F->Taken <= A->Strength * MarchRules{}.ForagePerManPerYear);
		VT_CHECK(F->Years >= 1);
	}
	const MarchStats M = W.Columns_();
	VT_CHECK_EQ(M.Bad, 0u);
